...
def get_inputs_and_targets(corpus_json_path, seq_length):
    
    with open(corpus_json_path, 'r') as json_f:
        levels = np.array(json.load(json_f))
    ...
    level_strs = []
    for level in levels:
        level_transposed = level.T
        level_str = ""
        for row in level_transposed:
            level_str += "".join([str(i) for i in row]) + "\n"
